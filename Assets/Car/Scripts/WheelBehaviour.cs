...
public class WheelBehaviour : MonoBehaviour
{
    ...
    {
        maxSuspensionDistance = car.suspensionRestDistance * 1.2f;
    ...
    private void HandleAccelerationForce()
    {
        ...
        wheelObject.Rotate(carSpeed + carSpeed * car.wheelHeight, 0, 0);

        if (!hasTraction) return;

        float normalizedCarSpeed = Mathf.Clamp01(Mathf.Abs(carSpeed) * 1.2f / car.topSpeed);
        float availableTorque = car.powerCurve.Evaluate(normalizedCarSpeed) * car.movementDirection.y * (car.horsePower / 30);

        if (car.movementDirection.y == 0)
            car.rb.AddForceAtPosition(-accelerationDirection * carSpeed * .5f, transform.position);
        else
